...
const parseHighlight = (text) => {
	...
	for (let i = 0; i < array.length; i += 2) {
		returnArr.push(
			<span key={uuidv4()}>{text.slice(lastSliceIndex, array[i].index)}</span>
		);
		returnArr.push(
			<span
				key={uuidv4()}
		...
		i + 2 >= array.length
			? returnArr.push(
					<span key={uuidv4()}>{text.slice(array[i + 1].index + 1)}</span>
			  )
			: (lastSliceIndex = array[i + 1].index + 1);
	}
	// console.log("returnArr", returnArr);
	return <p className="py-1">{returnArr}</p>;
};
	...
	return enemy["stats"][entry].special.map((skill) => {
		...
		if (skill.type === stat) {
			...
			return (
				<p className="py-1">
					<span
						className={`${specialModded ? "text-rose-600 font-semibold" : ""} `}
					>
		...
		if (skill.type === stat) {
			...
			if (skillToRender.hasOwnProperty("fixed")) {
				return (
					...
					>
						<span className={modded_skill && "text-rose-500 font-medium"}>
							{skillToRender["fixed"]}
						</span>
						{` (${skill.suffix[language]})`}
					</p>
