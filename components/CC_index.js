...
export default function CCIndex({ language, langPack }) {
	...
	for (const [index, { text, link }] of dailyStages.entries()) {
		dailyCCArr.push(
			<>
				<div className="border-b border-gray-500" key={link}>
					<div className="border-b border-gray-500 h-min w-full text-sm py-1">
						{`8/${index + 26}`}
					</div>
		...
		<div id="cc6" className="max-w-screen-sm md:max-w-3xl mb-24">
			...
			<table className="border border-gray-500 border-collapse w-full text-center align-middle">
				<tr className="border-y border-gray-500">
					<th className="py-2">{langPack.index.perma_stage}</th>
				</tr>
				<Link href={`/stages/cc/cc7-perma`}>
					<div className="border-y py-2 border-gray-500 text-base hover:cursor-pointer hover:bg-gray-600 text-indigo-300">
						{langPack.大骑士领酒吧街}
					</div>
				</Link>
				<tr>
					<th className="border-y border-gray-500 py-2">
