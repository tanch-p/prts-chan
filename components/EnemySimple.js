...
export default function EnemySimple({
    ...
    if (format !== "multiform") {
      if (row === 0) {
        ...
        switch (format) {
          ...
          case "powerup":
            return getSkills(enemy.id, enemy.powerup.special, stat, moddedStat);
        }
      }
    } else {
      return;
  ...
  const getSkills = (id, skills, stat, moddedStat) => {
      ...
      if (skill.type === stat) {
        ...
        if (skillToRender.hasOwnProperty("fixed")) {
          return (
            ...
            >
              <span className={modded_skill && "text-rose-500"}>{skillToRender["fixed"]}</span>
              {` (${skill.suffix[language]})`}
            </p>
          );
        }
        ...
        return (
          <p
            ...
            className={`whitespace-nowrap ${stat === "atk" ? "" : "px-2"}`}
          >
            <span className={modded_skill && "text-rose-500"}>
