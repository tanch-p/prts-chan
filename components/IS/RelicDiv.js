		...
		<div className="place-self-center">
			...
			<div className={`grid grid-cols-[repeat(6,140px)] gap-x-[1px]`}>
				{data.map((relic) => {
					...
					const someSelected = state[0].hasOwnProperty("key");
					return (
						<div
							key={relic.img}
							className={`group hover:cursor-pointer w-[140px] shadow-[0_0px_0_2px_#131313]
						${!selected ? "bg-[#303030] bg-opacity-40" : "bg-[#414141] bg-opacity-90"}
						${
							type === "hard"
								? !someSelected && !selected
									? "hover:bg-[#414141] hover:bg-opacity-50 "
									: ""
								: !selected && "hover:bg-[#414141] hover:bg-opacity-50"
						}`}
							onClick={() => {
								if (type === "hard") {
